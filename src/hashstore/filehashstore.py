...
class FileHashStore(HashStore):
    ...
    def _find_object(self, pid: str) -> dict[str, str]:
        ...
        content identifier. The `find_object` method validates the existence of
        an object based on the provided pid and returns the associated content
        identifier.
        ...
        err_msg = f"Pid reference file not found for pid ({pid}): {pid_ref_abs_path}"
        self.fhs_logger.error(err_msg)
    ...
    def _check_string(string: str, arg: str) -> None:
        """Check whether a string is None or empty - or if it contains an
        illegal character; throws an exception if so.
